...
<x-users.main.app-layout>
    ...
    <x-landlords.new-body>
        <form enctype="multipart/form-data" action="{{ route('help-centre.update', $frequently) }}" method="POST">
            @method('PUT')
            @csrf
            <x-common.input-select-100>
                <x-slot name="column">User Roles</x-slot>
                @if (isset($frequently->role_id ))
                    <x-slot name="type"><option value="{!! $frequently->userRole->id !!}">{!! $frequently->userRole->user_roles !!}</option></x-slot>
                @endif
